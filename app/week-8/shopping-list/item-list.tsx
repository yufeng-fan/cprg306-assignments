...
type Props = {
  items: ItemData[];
  onItemSelect: (mealName: string) => void;
};

type DisplayMode =
  | { mode: "list"; sorted: ItemData[] }
  | {
      mode: "group";
      grouped: Record<string, ItemData[]>;
      sortedCats: string[];
    };
  ...
  const display = useMemo((): DisplayMode => {
    if (sortBy === "group") {
      const grouped = props.items.reduce(
        (acc: Record<string, ItemData[]>, item) => {
        ...
        <ul>
          {display.sorted.map((item, index) => (
            <Item
              ...
              onSelect={() => props.onItemSelect(item.name)}
            />
          ))}
        </ul>
      )}
